...
export class ActivationService {
  ...
  activate(userId: string): Observable<ResponseInterface> {
    return this.http.get<ResponseInterface>(`${this.API_DOMAIN}/api/activate/${userId}`, httpOptions)
      .pipe(
  ...
  resendLink(userId: string): Observable<ResponseInterface> {
    return this.http.get<ResponseInterface>(`${this.API_DOMAIN}/api/activation-link/${userId}`, httpOptions)
      .pipe(
        retry(2), // retry a failed request up to 2 times
        catchError(this.handleError)
      );
