...
export class TabsService {
  ...
  // remove team member
  deleteTeamMember(memberId: string, teamId: string): Observable<ResponseInterface> {
    return this.http.get<ResponseInterface>(`${this.API_DOMAIN}/api/team/remove/${memberId}/${teamId}`, httpOptions)
      .pipe(
  ...
  // delete team
  deleteTeam(teamId: string): Observable<ResponseInterface> {
    return this.http.delete<ResponseInterface>(`${this.API_DOMAIN}/api/team/${teamId}`, httpOptions)
      .pipe(
        //retry(2), // retry a failed request up to 2 times
        catchError(this.handleError),
      );
