...
export class ViewService {
  ...
  // get all teams created by user without members details
  getTeams(id: string): Observable<ResponseInterface> {
    return this.http.get<ResponseInterface>(`${this.API_DOMAIN}/api/team/creator/${id}`, httpOptions)
      .pipe(
  ...
  // returns all teams created by user with members details
  getAllTeamsWithMember(id: string): Observable<ResponseInterface> {
    return this.http.get<ResponseInterface>(`${this.API_DOMAIN}/api/team/members/${id}`, httpOptions)
      .pipe(
        retry(2), // retry a failed request up to 2 times
        catchError(this.handleError),
      );
