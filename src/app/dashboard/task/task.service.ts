...
export class TaskService extends TaskClass {
  ...
  // retrieves task and its assignees attached 
  getTask(taskId: string): Observable<ResponseInterface> {
    return this.http.get<ResponseInterface>(`${this.API_DOMAIN}/api/task/${taskId}`, httpOptions)
      .pipe(
  ...
  // mark task
  markTaskAs(markObj): Observable<ResponseInterface> {
    return this.http.put<ResponseInterface>(`${this.API_DOMAIN}/api/task/markTaskAs`, markObj, httpOptions)
      .pipe(
        retry(2), // retry a failed request up to 2 times
        catchError(this.handleError)
      );
