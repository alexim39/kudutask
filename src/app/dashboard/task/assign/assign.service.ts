...
export class AssignService {
  ...
  assign(assignObj: AssignTaskInterface): Observable<ResponseInterface> {
    return this.http.put<ResponseInterface>(`${this.API_DOMAIN}/api/task/assign`, assignObj, httpOptions)
    .pipe(
  ...
  // remove assignee
  removeAssignee(removeObj: RemoveAssigneeInterface): Observable<ResponseInterface> {
    return this.http.put<ResponseInterface>(`${this.API_DOMAIN}/api/task/assign/remove`, removeObj, httpOptions)
    .pipe(
        //retry(2), // retry a failed request up to 2 times
        catchError(this.handleError),
    );
